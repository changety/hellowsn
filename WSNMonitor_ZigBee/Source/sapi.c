  ...
  if ( events & SYS_EVENT_MSG )
  {
    ...
    while ( pMsg )
    {
      switch ( pMsg->event )
      {
        ...
        case ZDO_MATCH_DESC_RSP_SENT:  //ZDO匹配描述符响应
...
void SAPI_Init( byte task_id )
{
...
#if ( SAPI_CB_FUNC )  //SAPI_CB_FUNVC 为TRUE
  ...
  afRegister( &sapi_epDesc );//在AF层注册该端口描述符
#endif

  // Turn off match descriptor response by default
  afSetMatch(sapi_epDesc.simpleDesc->EndPoint, FALSE);//默认关闭匹配描述符的响应

  // Register callback evetns from the ZDApp 返回注册时间
