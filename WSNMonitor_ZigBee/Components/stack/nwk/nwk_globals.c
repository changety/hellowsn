...
uint16 *Cskip;
  ...
  //uint8 CskipRtrs[MAX_NODE_DEPTH+1] = {5,5,5,5,5,0};
  //uint8 CskipChldrn[MAX_NODE_DEPTH+1] = {5,5,5,5,5,0};
  uint8 CskipRtrs[MAX_NODE_DEPTH+1] = {2,2,2,2,2,0};
  uint8 CskipChldrn[MAX_NODE_DEPTH+1] = {4,4,4,4,4,0};
...
void nwk_Status( uint16 statusCode, uint16 statusValue )
{
  ...
  switch ( statusCode )
  {
    ...
    case NWK_STATUS_ROUTER_ADDR:
     ...
     //   HalLedBlink (HAL_LED_2, 5, 50, 5);
         HalLedBlink( HAL_LED_2, 4, 50, (5000 / 4) );
      }
      break;
